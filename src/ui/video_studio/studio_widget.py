...
class VideoStudioWidget(QWidget):
    ...
    def _refresh_backend_info(self) -> None:
        ...
        self._backend_label.setText(b.label)
        installed = b.is_installed()
        self._backend_status.setText(
            "<span style='color:#15803d'>Installed.</span>"
            if installed else
            "<span style='color:#b91c1c'>Not installed — generate "
            "will fail. Click 'Install help' for setup steps.</span>")
    ...
    def _open_editor(self, scene_id: str) -> None:
        ...
        dlg = SceneEditorDialog(scene, rewrite_callback=rewrite_cb,
                                parent=self)
    ...
    def _ai_fill_from_chapter(self) -> None:
        ...
        dlg = AISceneGenerationDialog(
            ...
            default_duration=default_dur, parent=self)
        if dlg.exec() != QDialog.DialogCode.Accepted:
            return
    ...
    def _scene_output_dir(self, scene: Scene) -> Path:
        """Where this scene's clip files live on disk. Anchored to
        the project path when available; falls back to the user's
        home so the studio still works on an unsaved project."""
        if self._project is not None:
            proj_path = getattr(self._project, "project_path", None)
            if proj_path:
                return Path(proj_path) / "video_studio" / scene.id
        return Path.home() / ".writingaid_videos" / scene.id
